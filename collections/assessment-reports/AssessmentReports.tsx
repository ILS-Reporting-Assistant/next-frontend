...
export const AssessmentReports = () => {
  ...
  const handleDownloadDocx = async (report: Report) => {
    ...
    await downloadDocx(report.content, report.reportName || 'assessment-report')
  ...
  const handleDownloadPdf = async (report: Report) => {
    ...
    setDownloadingReportId(report._id)
    await downloadPdf(report.content, report.reportName || 'assessment-report')
    setDownloadingReportId(null)
  }
    ...
    },
    {
      ...
      render: (_: any, record: Report) => {
        ...
        const handleMenuClick = ({ key }: { key: string }) => {
          if (key === '1') {
            // View action
            router.push(`${ROUTE.VIEW_ASSESSMENT_REPORT}?reportId=${record._id}`)
          } else if (key === '2-docx') {
            // Download DOCX
