...
export const clientsService = {
  async getOrganizationClients(organizationId: string, query?: ClientsListQuery) {
    const { data } = await httpClient.get<ApiResponse<ClientsListResponse>>(
  ...
  async createClient(organizationId: string, payload: CreateClientPayload & { userId: string }) {
    const { data } = await httpClient.post<ApiResponse<CreateClientResponse>>(
  ...
  async updateClient(organizationId: string, clientId: string, payload: UpdateClientPayload) {
    const { data } = await httpClient.put<ApiResponse<UpdateClientResponse>>(
      ENDPOINT.CLIENTS.UPDATE(clientId),
      payload
    )
    return data
  },
  ...
  async deleteClient(organizationId: string, clientId: string) {
    const { data } = await httpClient.delete<ApiResponse<null>>(
      ENDPOINT.CLIENTS.DELETE(clientId)
    )
    return data
  },

  async getClient(organizationId: string, clientId: string) {
    const { data } = await httpClient.get<ApiResponse<Client>>(
